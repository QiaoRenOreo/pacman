...
void UI::update(std::vector<GameObjectStruct> objects)
{
	...
	for(auto &element : objects)
	{
		...
		{
			offsetx = (element.x-element.px)*((6*tween)-18);
			offsety = (element.y-element.py)*((6*tween)-18);

			if (tween == 1)
			{
				SDL_Rect dst = {(element.x*24)+offsetx, (element.y*24)+offsety, 24,24};
				SDL_RenderCopy(renderer, sheet, &clips3[element.type][element.dir], &dst);
			}
			if (tween == 0 or tween == 2)
			{
				SDL_Rect dst = {(element.x*24)+offsetx, (element.y*24)+offsety, 24,24};
				SDL_RenderCopy(renderer, sheet, &clips2[element.type][element.dir], &dst);
			}
		}
...
void UI::drawLives()
{
    // Draw amoutn of pacmans corresponding to the amount of lives
...
/*
 * Ininitialise the window and the renderer.
 */
void UI::init()
{
	// Init SDL
...
{
	// Load sprite sheet
	sheet = this->loadTexture("resources/sam_gfx.bmp",renderer);
...
void UI::loadMaps()
{
	...
	for(int i=0;i<10;i++)
	{
		...
		nr[UP] = 		{o+(size/2)*i,o+size*0,size/2,size/2};
		nr[DOWN] = 	nr[UP];
		nr[LEFT] = 	nr[UP];
		nr[RIGHT] = nr[UP];
	...
	pacman[UP] = 	{o+size*3,o+size*11,size,size};
	pacman[DOWN] = 	{o+size*15,o+size*7,size,size};
	pacman[LEFT] = 	{o+size*2,o+size*11,size,size};
	pacman[RIGHT] = {o+size*14,o+size*7,size,size};
	clips2[PACMAN] = pacman;

	pacman[UP] = 	{o+size*0,o+size*8,size,size};
	pacman[DOWN] = 	pacman[UP];
	pacman[LEFT] = 	pacman[UP];
	pacman[RIGHT] = pacman[UP];
	clips3[PACMAN] = pacman;

	pinky[UP] = 	{o+size*7,o+size*9,size,size};
	pinky[DOWN] = 	{o+size*3,o+size*9,size,size};
	pinky[RIGHT] = 	{o+size*1,o+size*9,size,size};
	pinky[LEFT] = 	{o+size*5,o+size*9,size,size};
	clips2[PINKY] = pinky;

	blinky[UP] = 	{o+size*7,o+size*7,size,size};
	blinky[DOWN] = 	{o+size*3,o+size*7,size,size};
	blinky[RIGHT] = {o+size*1,o+size*7,size,size};
	blinky[LEFT] =  {o+size*3,o+size*7,size,size};
	clips2[BLINKY] = blinky;

	clyde[UP] = 	{o+size*7,o+size*10,size,size};
	clyde[DOWN] = 	{o+size*3,o+size*10,size,size};
	clyde[RIGHT] = 	{o+size*1,o+size*10,size,size};
	clyde[LEFT] =  	{o+size*5,o+size*10,size,size};
	clips2[CLYDE] = clyde;

	inky[UP] = 		{o+size*15,o+size*9,size,size};
	inky[DOWN] = 	{o+size*11,o+size*9,size,size};
	inky[LEFT] = 	{o+size*13,o+size*9,size,size};
	inky[RIGHT] =  	{o+size*9,o+size*9,size,size};
	clips2[INKY] = inky;

	scared[UP] = 		{o+size*13,o+size*6,size,size};
	scared[DOWN] = 		scared[UP];
	scared[LEFT] = 		scared[UP];
	scared[RIGHT] =  	scared[UP];
	clips2[SCARED] = 	scared;

	scaredinv[UP] = 		{o+size*5,o+size*11,size,size};
	scaredinv[DOWN] = 		scaredinv[UP];
	scaredinv[LEFT] = 		scaredinv[UP];
	scaredinv[RIGHT] =  	scaredinv[UP];
	clips2[SCAREDINV] = 	scaredinv;
